...
export const DataQualitySummary: React.FC<DataQualitySummaryProps> = ({ issues, loading }) => {
  ...
  return (
    <section
      className={`quality-summary ${open ? 'open' : ''} ${issues.length ? 'has-issues' : 'clean'}`}
    >
      <button
      ...
      >
        <span className="analysis-toggle-title">
          <span className="analysis-icon">◇</span>Data quality
        </span>
          ...
          {issues.length ? (
            issues.map((issue, index) => (
              <div
                ...
                key={`${issue.kind}-${issue.column ?? ''}-${index}`}
              >
                <span>
                  {issue.kind === 'duplicates' ? '⧉' : issue.kind === 'nulls' ? '◌' : '◇'}
                </span>
                <p>{issue.message}</p>
              </div>
            ))
          ) : (
            <p className="quality-clean">
              No nulls, duplicate rows, or IQR outliers were detected.
            </p>
          )}
        </div>
      )}

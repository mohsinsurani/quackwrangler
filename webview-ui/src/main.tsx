...
if (isDevelopment && typeof acquireVsCodeApi === 'undefined') {
  ...
  window.setTimeout(() => {
    ...
    window.postMessage({
      ...
      quality: {
        issues: [
          { severity: 'warning', kind: 'nulls', message: 'attendance contains 34 null values', column: 'attendance', count: 34 },
          { severity: 'info', kind: 'outliers', message: 'revenue contains 12 potential outliers', column: 'revenue', count: 12 },
        ],
      },
    }, '*');
    window.postMessage({
